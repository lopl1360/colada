    ...
    with patch.object(rl, "SentimentAnalyzer", return_value=DummySentiment()):
        with patch.object(rl, "LSTMPricePredictor", DummyModel):
            with patch.object(rl, "torch", DummyTorch()):
                with patch.object(
                    rl.order_submitter, "submit_bracket_order"
                ) as mock_submit:
                    with patch.object(
                        rl.position_manager, "get_open_position", return_value=None
                    ):
                        with patch.object(
                            rl.volatility_manager,
                            "compute_stop_target",
                            ...
                            with patch.object(
                                ...
                                with patch.object(
                                    rl, "_fetch_equity", return_value=10000
                                ):
                                    with patch.object(
                                        rl.time, "sleep", return_value=None
                                    ):
                                        rl.run_trade_loop(
                                            symbol="AAPL",
                                            seq_len=1,
